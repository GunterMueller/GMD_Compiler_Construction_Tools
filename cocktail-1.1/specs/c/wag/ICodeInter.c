...
int NewLabel ()
{
   if (++ LC == LocSize) ExtendArray ((char * *) & LocPtr, & LocSize, sizeof (int));
   return LC;
}
...
void ResolveLabels ()		/* replace symbolic labels by concrete ones */
{
  register int pc;
  for (pc = 0; pc < PC; pc ++)
    switch (Code [pc].OP) {
...
void Interpret ()
{
  ...
  do {
    ...
    case RET:
      ...
      break;
    case CHK:
      if (Store [SP].Value.Vi < P || Store [SP].Value.Vi > Q.qc)
