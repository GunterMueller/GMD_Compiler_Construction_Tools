...
# endif
{
  ...
  if (index == Codes_cNoIndex) {
    ...
    Sets_Exclude(&todo, (LONGCARD)Scanner_Epsilon);
...
# endif
{
  ...
  Sets_MakeSet(&set, (LONGCARD)Codes_MaxCode());
  {
          ...
          if (B_11 <= B_12)
            for (no = B_11;; no += 1) {
              ...
              for (bit = 0; bit <= cBitsPerLong - 1; bit += 1) {
                ...
                if (Sets_IsElement((LONGCARD)(no * cBitsPerLong + bit), &set)) {
                  INC1(value, bitvalue);
                }
                bitvalue = bitvalue / 2;
                if (bit % 4 == 3) {
...
# endif
{
  ...
  for (bit = 0; bit <= cBitsPerChar - 1; bit += 1) {
    Sets_MakeSet(&sets.A[bit], (LONGCARD)Codes_MaxCode());
    ...
    if (B_13 <= B_14)
      for (code = B_13;; code += 1) {
        ...
        for (bit = cBitsPerChar - 1; bit >= 0; bit += -1) {
          ...
          if (Sets_IsElement((LONGCARD)code, &sets.A[bit])) {
            INC1(value, bitvalue);
          }
          if (bit % 4 == 0) {
            IO_WriteC(file, Hex.A[value]);
...
# endif
{
  ...
  Sets_MakeSet(&labels, (LONGCARD)Idents_MaxIdent());
  Spaces();
  IO_WriteS(file, (STRING)"for (;;) {", 10L);
  IO_WriteNl(file);
  INC(SpaCo);
...
# endif
{
  ...
  Sets_MakeSet(&s, (LONGCARD)Codes_MaxCode());
  {
    ...
    if (B_27 <= B_28)
      for (l = B_27;; l += 1) {
        if (Sets_IsElement(l, &labels)) {
          Sets_Include(&s, (LONGCARD)Codes_Code((SHORTCARD)l));
        }
        if (l >= B_28) break;
      }
  }
