...
# endif
{
  ...
  case Tree_Option:;
    ...
    Sets_Include(first, (LONGCARD)Scanner_Epsilon);
    break;
  case Tree_Times:;
  ...
  case Tree_List:;
    ...
    if (Sets_IsElement((LONGCARD)Scanner_Epsilon, first)) {
      Sets_Exclude(first, (LONGCARD)Scanner_Epsilon);
    ...
    break;
  case Tree_Action:;
    Sets_AssignElmt(first, (LONGCARD)Scanner_Epsilon);
    break;
  case Tree_Leaf:;
    Sets_AssignElmt(first, (LONGCARD)t->U_1.V_38.Leaf.id->U_1.V_23.Id.ident);
...
# endif
{
  ...
  Sets_MakeSet(&N, (LONGCARD)Idents_MaxIdent());
  Sets_MakeSet(&F, (LONGCARD)Idents_MaxIdent());
...
# endif
{
  ...
  Sets_MakeSet(&Fx, (LONGCARD)Idents_MaxIdent());
  Sets_MakeSet(&Fy, (LONGCARD)Idents_MaxIdent());
  Sets_MakeSet(&Y, (LONGCARD)Idents_MaxIdent());
  ...
  while (!Sets_IsEmpty(Y)) {
    y = Sets_Extract(&Y);
    if (y == x) {
      Sets_Include(&vLeftRec, (LONGCARD)y);
    } else if (Sets_IsElement((LONGCARD)y, G_3_X)) {
      Sets_Exclude(G_3_X, (LONGCARD)y);
      Traverse(y);
    }
    ...
    Sets_Exclude(&Fy, (LONGCARD)Scanner_Epsilon);
    Sets_Union(&Fx, Fy);
  }
  Table_SetFirst(x, Fx);
  if ((*G_2_N)->A[x] == d) {
...
# endif
{
  ...
  Sets_MakeSet(&vLeftRec, (LONGCARD)Idents_MaxIdent());
  ...
  Sets_MakeSet(&Terms, (LONGCARD)Idents_MaxIdent());
  Types_Terminals(&Terms);
  Sets_MakeSet(&Nonterms, (LONGCARD)Idents_MaxIdent());
  Types_Nonterminals(&Nonterms);
  Sets_MakeSet(&X, (LONGCARD)Idents_MaxIdent());
...
# endif
{
  return Sets_IsElement((LONGCARD)nt, &vLeftRec);
}
...
# endif
{
  ...
  Sets_MakeSet(&t->U_1.V_32.expr.first, (LONGCARD)Idents_MaxIdent());
  switch (t->U_1.V_1.Kind) {
  case Tree_Option:;
    {
      ...
      Sets_Include(&W_1->first, (LONGCARD)Scanner_Epsilon);
    }
    break;
  case Tree_Times:;
    {
      ...
      Sets_Include(&W_2->first, (LONGCARD)Scanner_Epsilon);
    }
    break;
  case Tree_Plus:;
    {
  ...
  case Tree_List:;
    {
      ...
      TreeFirst(W_4->sep);
      if (Sets_IsElement((LONGCARD)Scanner_Epsilon, &W_4->first)) {
        Sets_Exclude(&W_4->first, (LONGCARD)Scanner_Epsilon);
  ...
  case Tree_Action:;
    {
      ...
      Sets_AssignElmt(&W_5->first, (LONGCARD)Scanner_Epsilon);
    }
    break;
  case Tree_Leaf:;
    {
  ...
  case Tree_Sequence0:;
    {
      ...
      Sets_AssignElmt(&W_9->first, (LONGCARD)Scanner_Epsilon);
    }
    break;
  case Tree_Sequence:;
    {
      ...
      if (Sets_IsElement((LONGCARD)Scanner_Epsilon, &W_10->first)) {
        Sets_Exclude(&W_10->first, (LONGCARD)Scanner_Epsilon);
