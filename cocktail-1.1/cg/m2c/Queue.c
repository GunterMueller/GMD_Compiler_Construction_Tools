...
# endif
{
  ...
  QueueIn = 0;
  QueueOut = 0;
  Sets_MakeSet(&QueueSet, (LONGCARD)QueueSize);
}
...
# endif
{
  if (!Sets_IsElement((LONGCARD)Elmt, &QueueSet)) {
    QueuePtr->A[QueueIn] = Elmt;
    QueueIn = (QueueIn + 1) % QueueSize;
    INC(QueueCount);
    Sets_Include(&QueueSet, (LONGCARD)Elmt);
  }
}
...
# endif
{
  ...
  DEC(QueueCount);
  Sets_Exclude(&QueueSet, (LONGCARD)Result);
  return Result;
}
...
# endif
{
  return Sets_IsElement((LONGCARD)Elmt, &QueueSet);
}
